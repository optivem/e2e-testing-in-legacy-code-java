...
$networkTest = docker compose exec -T monolith wget -q -O- http://erp-api:3000/products/10 2>&1
...
try {
    ...
    docker compose logs monolith --tail=100 | Select-String -Pattern "Exception:|ERROR|Caused by:|Failed to" -Context 2,10
    Write-Host ""
